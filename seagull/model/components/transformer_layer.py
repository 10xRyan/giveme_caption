...
class TransformerLayer(Module):
    ...
    def forward(
        ...
        """
        # TODO-6.1
        res1 = input_embeddings
        x = self.maybe_apply_layer_norm(input_embeddings, "pre")
        x, attn_probs = self.mha(x, use_kv_cache=use_kv_cache)
        x = res1 + x
        x = self.maybe_apply_layer_norm(x, "post")

        res2 = x
        x = self.maybe_apply_layer_norm(x, "pre")
        x = self.ffn(x)
        x = res2 + x
        x = self.maybe_apply_layer_norm(x, "post")

        return (x, attn_probs) # tuple of outputs and masked attn probs
