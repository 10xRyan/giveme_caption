...
class SeagullLM(Module):
    ...
    def forward(
        ...
        # TODO-6.2
        x = self.seagull(input_ids, 
                          position_ids, 
                          padding_mask, 
                          use_kv_cache, 
                          return_output_at_all_layers, 
                          return_attentions)

        outputs = x if not return_attentions else x[0]
        outputs = outputs[-1]

        logits = None
        if self.weight_tying == True:
          tok_embed_weights = self.seagull.embedding.token_embedding.weight.data
          logits = outputs @ tok_embed_weights.T
        else:
