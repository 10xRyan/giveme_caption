...
class MultiHeadAttention(Module):
    ...
    def masked_attention_probs(
        ...
        # TODO-4.1
        d = torch.tensor([query.shape[1]], device=query.device)
        probs = query @ key.transpose(2, 3) / torch.sqrt(d)
        inf = float("-inf")
        if padding_mask is not None:
          padding_mask = padding_mask[:, None, None, :]
          probs = probs.masked_fill_(padding_mask, inf)
        
        probs = self._apply_causal_mask(probs)
        return softmax(probs, dim=3,
        dropout_proba=self.attn_dropout_proba, 
    ...
    def attention(masked_attn_probs: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        """See: https://pages.github.coecis.cornell.edu/cs4740/hw4-fa23/seagull.nn.transformer.mha.html."""
        # TODO-4.2
        return masked_attn_probs @ value  
    def forward(
        self,
